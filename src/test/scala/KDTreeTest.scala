...
class DimOrderingTest extends AnyFlatSpec with should.Matchers {
...
class NearestNeighborTest extends AnyFlatSpec with should.Matchers {
  ...
  it should "build a tree of n points" in {
    ...
    val tree =
      KDTree.fromSeq(points)(DimensionalOrdering.dimensionalOrderingForSeq[Seq[Int], Int](2))
    tree.size should equal(4)
  ...
  it should "build a large tree" in {
    ...
    for { x <- 2 to (99, 10); y <- 2 to (99, 10) } {
      ...
      near.slice(5, 9).toSet should equal(
        Set((x - 1, y - 1), (x - 1, y + 1), (x + 1, y - 1), (x + 1, y + 1)))
    }

    t.findNearest((100, 1), 4).toSet should equal(Set((100, 1), (100, 2), (99, 1), (99, 2)))

    t.findNearest((100, 100), 4).toSet should equal(Set((100, 100), (99, 99), (100, 99), (99, 100)))

    t.findNearest((27, 0), 4).toSet should equal(Set((27, 1), (26, 1), (28, 1), (27, 2)))

    t.regionQuery(new RegionBuilder[(Int, Int)] {
      from((7, 7), 1)
    })

    t.regionQuery(Region.from((73, 73), 1)).toSet should equal(
      (for {
        x <- 1 to 100; y <- 73 to 100
      } yield (x, y)).toSet)

    t.regionQuery(Region.to((17, 73), 0)).toSet should equal(
      (for {
        x <- 1 to 17; y <- 1 to 100
      } yield (x, y)).toSet)
...
class KDTreeMapTest extends AnyFlatSpec with should.Matchers {
  val map = KDTreeMap((3, 5) -> "a", (7, 6) -> "b", (12, 7) -> "c")
